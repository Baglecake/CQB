...
from enhanced_rao_context_manager import EnhancedRAOContextManager as CQBContextManager
...
class CQBModelManager:
    ...
    def generate_text(self, model_id: str, prompt: str, temperature: float = None, max_tokens: int = None) -> str:
        ...
        base_sampling_params = self.sampling_params[model_id]

        # Create new sampling params with any overrides
        from vllm import SamplingParams

        sampling_params = SamplingParams(
            temperature=temperature if temperature is not None else base_sampling_params.temperature,
            top_p=base_sampling_params.top_p,
            max_tokens=max_tokens if max_tokens is not None else base_sampling_params.max_tokens,
...
class DynamicAgentGenerator:
    ...
    def __init__(self, model_manager: CQBModelManager, rao_config: Dict = None):
        ...
        if self.rao_config.get('enabled', False):
            # Pass the model manager
            self.context_manager = CQBContextManager(
                self.model_manager,  # Add this line
    ...
    def analyze_and_generate_agents(self, query: str, max_agents: int = 8) -> List[CQBAgent]:
        ...
        context_content = None # Initialize context_content
        domain_analysis = None # Initialize domain_analysis
        ...
        if self.rao_config.get('enabled', False) and self.context_manager:
            ...
            if context_content:
                ...
                # Ensure domain_analysis is set here when context_analysis is available
                if context_analysis:
                    domain_analysis = {
                        "primary_domain": context_analysis.get("domain_focus", "general"),
                        "complexity_level": context_analysis.get("complexity_level", "medium"),
                        "key_concepts": context_analysis.get("key_concepts", [])
                    }

                print(f"✅ RAO: Context analysis complete - Domain: {context_analysis.get('domain_focus', 'general')}")
        ...
        # Pass context_analysis and context_content to _extract_agent_specs
        agent_specs = self._extract_agent_specs(analysis, context_analysis, max_agents, query, context_content, domain_analysis) # Pass domain_analysis here
...
Context Analysis:
- Primary Domain: {context_analysis.get('domain_focus', 'general')}
- Complexity Level: {context_analysis.get('complexity_level', 'medium')}
- Key Concepts: {', '.join(context_analysis.get('key_concepts', [])[:10])}
- Suggested Specialists: {', '.join(context_analysis.get('specialties_needed', [])[:5])}

Context Summary:
{context_analysis.get('context_summary', '')[:500]}...
...
Format your response as a simple list of expert types, one per line."""
    ...
    def _extract_agent_specs(self, analysis: str, context_analysis: Dict = None,
                       max_agents: int = 8, query: str = "", context_content: str = None, domain_analysis: Dict = None) -> List[CQBAgentSpec]: # Added domain_analysis parameter
        ...
        if context_analysis and context_analysis.get('specialties_needed'):
            context_specialists = context_analysis['specialties_needed'][:max_agents//2]
            # Prioritize context specialists by adding them to the beginning
            expert_types = context_specialists + [e for e in expert_types if e not in context_specialists]
        ...
        if len(expert_types) < max_agents:
            general_experts = [
                ...
                "Data Analyst", "Research Methodologist"
            ]
            # Add general experts if needed, avoiding duplicates
            for expert in general_experts:
                if len(expert_types) < max_agents and expert not in expert_types:
                    expert_types.append(expert)
        ...
        # Ensure domain_analysis is a valid dictionary before using it
        effective_domain_analysis = domain_analysis if domain_analysis is not None else {
            "extraction_results": [],
            "domain_focus": "general",
            "complexity_level": "medium",
            "key_concepts": [],
            "extraction_metadata": {"extraction_method": "fallback_default"}
        }
        ...
        for i, expert_type in enumerate(selected_experts):
            ...
            # Build specialist-specific context summary for THIS agent
            specialist_context = ""
            # Access context_manager and context_analysis directly
            if self.rao_config.get('enabled', False) and self.context_manager and context_content:
                 # Check if context_analysis is not None before accessing its keys
                 if context_analysis:
                     specialist_context = self.context_manager._build_agent_context_summary(
                        context_analysis.get('extraction_results', []),
                        query,  # Pass the actual query
                        effective_domain_analysis, # Pass the potentially defaulted domain_analysis
                        expert_type  # Pass the specific specialty
                     )
                 else:
                      # If context_analysis is None, build a generic context summary
                      specialist_context = self.context_manager._build_generic_context_summary(
                          [], # No extractions if context_analysis is None
                          query,
                          effective_domain_analysis # Pass the potentially defaulted domain_analysis
                      )
            ...
            spec = CQBAgentSpec(
                ...
                context_summary=specialist_context  # Use the specialist context!
            )
